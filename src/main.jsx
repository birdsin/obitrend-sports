...
function MatchCard({m,onClick,favorite,onFavorite}){return <article className="match-card" onClick={onClick} role="button" tabIndex="0" onKeyDown={e=>{if(e.key==="Enter"||e.key===" ")onClick()}}><div className="league">{m.league}<button className={"fav-btn "+(favorite?"fav-active":"")} onClick={e=>{e.stopPropagation();onFavorite()}} aria-label="Favorite">{favorite?"★":"☆"}</button><span className={m.live?"live-dot":"scheduled"}>{m.live?"LIVE":m.time}</span></div><div className="teams"><div><strong>{m.home}</strong>{m.live&&<b>{m.hs}</b>}</div><div><strong>{m.away}</strong>{m.live&&<b>{m.as}</b>}</div></div><div className="card-footer">{m.live?<><span>{m.time}</span><span>Match Centre →</span></>:<span>View match →</span>}</div></article>}
...
function MatchesPage({filtered,query,setQuery,setSelected,leagueId,setLeagueId,season,setSeason,standings,setStandings,standingsLoading,setStandingsLoading,setProfile,isFavorite,toggleFavorite}){
  ...
  return <><SectionTitle title="All matches"/><SearchBox query={query} setQuery={setQuery}/><div className="match-grid">{filtered.map(m=><MatchCard key={m.id} m={m} onClick={()=>setSelected(m)} favorite={isFavorite("match",m.id)} onFavorite={()=>toggleFavorite({type:"match",id:m.id,name:m.home+" vs "+m.away})}/>)}</div>
...
function ProfileModal({profile,onClose,favorite,onFavorite}){
  ...
  return <div className="modal-backdrop" onClick={onClose}><div className="modal profile-modal" onClick={e=>e.stopPropagation()}><button className="close" onClick={onClose}><X/></button><span className="live-label">{profile.type==="team"?"TEAM PROFILE":"PLAYER PROFILE"}</span><button className={"profile-follow "+(favorite?"follow-active":"")} onClick={onFavorite}>{favorite?"★ Following":"☆ Follow"}</button>{loading?<div className="detail-empty">Loading profile…</div>:profile.type==="team"?<TeamProfile data={data} setProfile={setProfileFromChild}/>:<PlayerProfile data={data}/>}</div></div>;
